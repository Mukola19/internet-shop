module.exports = class DeviceAllDto {
    ...
    constructor(model, basketId) {
      ...
      if (model.impression) {
        this.impression = []
        model.impression.forEach(({ id, advantages, createdAt, description, rate, shortcomings, email }) => {
          console.log(email);
          this.impression.push({ id, advantages, createdAt, description, rate, shortcomings, email })
        })
